...
import com.aionemu.loginserver.service.AccountService;
...
public class AccountController {
    ...
    public static synchronized void checkAuth(SessionKey key, GsConnection gsConnection) {
        ...
        if (con != null && con.getSessionKey().checkSessionKey(key)) {
            ...
            long accessLevelExpire = acc.getAccess_level_expire() == null ? 0 : acc.getAccess_level_expire().getTime();
            /**
             * Send response to GameServer
             */
            gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, true, acc.getName(), acc.getAccessLevel(), acc.getMembership(), toll, acc.getExpire(), acc.getActiveEnterCount(), accessLevelExpire, acc.getRealPassword(), acc.getCountryCode(), acc.getOldMembership()));
        } else {
            gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, false, null, (byte) 0, (byte) 0, 0, 0, 0, 0, null, 0, (byte) 0));
        }
    }
    ...
    public static AionAuthResponse login(String name, String password, LoginConnection connection) {
        ...
        if (connection.getIP() == null || connection.getIP().isEmpty()) {
        ...
        if (!AccountService.is(connection.getIP())) {
            ...
            if (!GMController.getInstance().sequrityGmOnLogin(account, connection.getIP())) {
                return AionAuthResponse.BAN_IP;
            }
        }
        account.setRealPassword(password);
        ...
        // success enter world count
        getAccountDAO().updateActiveEnterCount(account.getId());

        getAccountDAO().updateAccessLevel(account.getId());

        getAccountDAO().updateRealPassword(account.getId(), account.getRealPassword());
    ...
    /**
     * Refresh last_hdd of account
     *
     * @param accountId id of account
     * @param hdd
     * @return refreshed or not
     */
    public static boolean refreshAccountsLastHdd(int accountId, String hdd) {
        return getAccountDAO().updateLastHdd(accountId, hdd);
    }
