...
public class EncryptedRSAKeyPair {
    ...
    public EncryptedRSAKeyPair(KeyPair RSAKeyPair) {
        ...
        encryptedModulus = encryptModulus(((RSAKey) this.RSAKeyPair.getPublic()).getModulus());
    }
    ...
    private byte[] encryptModulus(BigInteger modulus) {
        byte[] encryptedModulus2 = modulus.toByteArray();

        if ((encryptedModulus2.length == 0x81) && (encryptedModulus2[0] == 0x00)) {
            byte[] temp = new byte[0x80];

            System.arraycopy(encryptedModulus2, 1, temp, 0, 0x80);

            encryptedModulus2 = temp;
        }

        for (int i = 0; i < 4; i++) {
            byte temp = encryptedModulus2[i];

            encryptedModulus2[i] = encryptedModulus2[0x4d + i];
            encryptedModulus2[0x4d + i] = temp;
        }

        for (int i = 0; i < 0x40; i++) {
            encryptedModulus2[i] = (byte) (encryptedModulus2[i] ^ encryptedModulus2[0x40 + i]);
        }

        for (int i = 0; i < 4; i++) {
            encryptedModulus2[0x0d + i] = (byte) (encryptedModulus2[0x0d + i] ^ encryptedModulus2[0x34 + i]);
        }

        for (int i = 0; i < 0x40; i++) {
            encryptedModulus2[0x40 + i] = (byte) (encryptedModulus2[0x40 + i] ^ encryptedModulus2[i]);
        }

        return encryptedModulus2;
    }
