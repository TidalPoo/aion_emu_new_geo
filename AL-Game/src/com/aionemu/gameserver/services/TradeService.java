...
import com.aionemu.gameserver.configs.main.TradeConfig;
...
public class TradeService {
    ...
    public static int DEFAULT_SELL_MODIFICATOR = 100;
    // for admin cmd
    // public static float SellMultipler = 100f; // dont use this. only for dev version
    ...
    public static boolean performBuyFromShop(Npc npc, Player player, TradeList tradeList) {
        ...
        for (TradeItem tradeItem : tradeList.getTradeItems()) {
            ...
            //player.addItem().setBuyItems(true);
            long count = ItemService.addItem(player, tradeItem.getItemTemplate().getTemplateId(), tradeItem.getCount(), AddItemType.BUY, "npcid: " + npc.getNpcId());
            if (count != 0) {
    ...
    private static boolean validateBuyItems(Npc npc, TradeList tradeList, Player player) {
        ...
        for (TradeItem tradeItem : tradeList.getTradeItems()) {
            ...
            Item item = player.getInventory().getItemByObjId(tradeItem.getItemId());
            if (item != null && item.isGMAddItems()) {
                PacketSendUtility.sendBrightYellowMessageOnCenter(player, "Нельзя продавать прописанные предметы!");
                return false;
            }
        }
        return true;
    ...
    public static boolean performSellToShop(Player player, TradeList tradeList) {
        ...
        for (TradeItem tradeItem : tradeList.getTradeItems()) {
            ...
            if (item.getItemCount() - tradeItem.getCount() < 0) {
                ...
                repurchaseItem = ItemFactory.newItem(item.getItemId(), tradeItem.getCount(), AddItemType.forAllTypes(item.getAddItems()), item.getOwner());
    ...
    public static boolean performBuyFromTradeInTrade(Player player, int npcObjectId, int itemId, int count) {
        ...
        ItemService.addItem(player, itemId, count, AddItemType.BUY, "npcid: " + npcId);
        return false;
    }
    ...
    public static boolean performSellForAPToShop(Player player, TradeList tradeList, TradeListTemplate purchaseTemplate) {
        ...
        for (TradeItem tradeItem : tradeList.getTradeItems()) {
            ...
            float sellMultipler = getSellModificator(purchaseTemplate.getNpcId()) / 100f;
            if (inventory.decreaseByObjectId(itemObjectId, count)) {
                AbyssPointsService.addAp(player, (int) (item.getItemTemplate().getAcquisition().getRequiredAp() * (int) count * sellMultipler));
            }
        }
        return true;
    }

    public static int getSellModificator(int npcId) {
        int mod = DEFAULT_SELL_MODIFICATOR;
        try {
            NpcTemplate trader = DataManager.NPC_DATA.getNpcTemplate(npcId);
            switch (trader.getTitleId()) {
                case 463230:
                    mod = TradeConfig.SELL_MULTIPLER_TITLE_463230;
                    break;
                case 463495:
                    mod = TradeConfig.SELL_MULTIPLER_TITLE_463495;
                    break;
                case 463628:
                    mod = TradeConfig.SELL_MULTIPLER_TITLE_463628;
                    break;
                case 463224:
                    mod = TradeConfig.SELL_MULTIPLER_TITLE_463224;
                    break;
            }
        } catch (Exception exc) {
