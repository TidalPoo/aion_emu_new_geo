...
/**
 * @author ATracer modified cura, Alex
 */
public class RenameService {
    ...
    public static boolean renamePlayer(Player player, String oldName, String newName, int item) {
        ...
        //NEW WEDDINGS
        WeddingService.getInstance().getRenamePartner(player);
        return true;
    }
    ...
    public static boolean renameLegion(Player player, String name, int item) {
        ...
        //1400160
        //Нельзя менять название легиона, владеющего крепостью или артефактом.
        if (player.getLegion() != null) {
            for (Map.Entry<Integer, ArtifactLocation> a : SiegeService.getInstance().getArtifacts().entrySet()) {
                if (a.getValue().getLegionId() != 0 && a.getValue().getLegionId() == player.getLegion().getLegionId()) {
                    PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400160));
                    return false;
                }
            }
            for (Map.Entry<Integer, FortressLocation> f : SiegeService.getInstance().getFortresses().entrySet()) {
                if (f.getValue().getLegionId() != 0 && f.getValue().getLegionId() == player.getLegion().getLegionId()) {
                    PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400160));
                    return false;
                }
            }
