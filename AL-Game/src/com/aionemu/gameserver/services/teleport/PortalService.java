...
public class PortalService {
    ...
    public static void port(final PortalPath portalPath, final Player player, int npcObjectId) {
        ...
        if (instanceRaceReq && portalPath.getRace().getRaceId() < 2 && !player.getRace().equals(portalPath.getRace())) {
        ...
        int siegeId = portalPath.getSigeId();
        if (instanceRaceReq && siegeId != 0) {
            if (!checkSigeId(player, siegeId)) {
                PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_MOVE_PORTAL_ERROR_INVALID_RACE);
                return;
            }
        }

        if (instanceRaceReq && siegeId != 0 && portalPath.isLegion()) {
            if (!checkLegion(player, siegeId)) {
                return;
            }
        }

        // FIX IT FOR FFA AND TVT ZONE ENTER
        if (player.getWorldId() == WorldMapType.SANCTUM.getId()
                || player.getWorldId() == WorldMapType.PANDAEMONIUM.getId()) {
            instanceRaceReq = false;
        }
    ...
    private static boolean checkLegion(Player player, int siegeId) {
        ...
        if (loc != null) {
            ...
            if (loc.getLegionId() != player.getLegion().getLegionId()) {
                return false;
            }
        }
        return true;
    ...
    private static boolean checkRace(Player player, Race portalRace) {
        return portalRace.getRaceId() < 2 && player.getRace().equals(portalRace) || portalRace.equals(Race.PC_ALL);
    }
    ...
    private static void easyTransfer(Player player, PortalLoc loc) {
        TeleportService2.teleportTo(player, loc.getWorldId(), loc.getX(),
                loc.getY(), loc.getZ(), loc.getH(), TeleportAnimation.JUMP_AIMATION_2);
    }
}
