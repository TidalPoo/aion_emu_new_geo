...
public class PrivateStoreService {
    ...
    public static void addItems(Player activePlayer, TradePSItem[] tradePSItems) {
        ...
        if (CreatureState.ACTIVE.getId() != activePlayer.getState()) {
            PacketSendUtility.sendMessage(activePlayer, "your creature state not validate for open private store");
            return;
        }
    ...
    private static void createStore(Player activePlayer) {
        if (activePlayer.isInState(CreatureState.RESTING) || activePlayer.isInPlayerMode(PlayerMode.RIDE)) {
            return;
        }
    ...
    public static void sellStoreItem(Player seller, Player buyer, TradeList tradeList) {
        ...
        /*if (!RestrictionsManager.canPrivateStore(buyer)) {
         return;
         }*/
        if (buyer.getLevel() < CustomConfig.BUY_PRIVATE_SHOP_LEVEL) {
            PacketSendUtility.sendPacket(buyer, new SM_SYSTEM_MESSAGE(1400399, CustomConfig.BUY_PRIVATE_SHOP_LEVEL));
            return;
        }
        ...
        if (freeSlots < tradeList.size()) {
            PacketSendUtility.sendPacket(buyer, new SM_SYSTEM_MESSAGE(1390182));
            return;
        }
        /**
            ...
            for (TradeItem tradeItem : tradeList.getTradeItems()) {
                ...
                if (item != null) {
                    ...
                    if (item.isGMAddItems()) {
                        PacketSendUtility.sendMessage(buyer, "Нельзя обмениваться прописанными предметами.");
                        return;
                    }

                    // Decrease/remove item from store and add them to buyer
                    decreaseItemFromPlayer(seller, item, tradeItem);
                    ItemService.addItem(buyer, item.getItemId(), tradeItem.getCount(), item, AddItemType.PRIVATESTORE, "seller: " + seller.getName() + "(" + seller.getObjectId() + ") buyer: " + buyer.getName() + "(" + buyer.getObjectId() + ")");
