...
public class ItemRemodelService {
    ...
    public static void remodelItem(Player player, int keepItemObjId, int extractItemObjId) {
        ...
        //set expire time extract item to removel item
        if (extractItem.getExpireTime() != 0 && keepItem.getExpireTime() == 0) {
            //TODO fix it!!!
            keepItem.setExpireTime(extractItem.getExpireTime());
            keepItem.setSkinExpire(true);
            ExpireTimerTask.getInstance().addTask(keepItem, player);
        }
