        ...
        switch (type) {
            ...
            case ACCOUNT_WAREHOUSE:
                if (!item.isStorableinAccWarehouse(player)) {
                    ...
                    return true;
                } else if (item.isGMAddItems()) {
                    PacketSendUtility.sendYellowMessageOnCenter(player, "Нельзя класть прописанный предмет на склад аккаунта!");
                    return true;
                }
                break;
            case LEGION_WAREHOUSE:
                if (!item.isStorableinLegWarehouse(player) || !LegionConfig.LEGION_WAREHOUSE) {
                    ...
                    PacketSendUtility.sendYellowMessageOnCenter(player, "Нельзя класть прописанный предмет на склад легиона!");
                    return true;
                }
                break;
        }
