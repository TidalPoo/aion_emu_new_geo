...
public class StigmaService {
    ...
    public static boolean extendAdvancedStigmaSlots(Player player) {
        ...
        if (newAdvancedSlotSize <= 6) { // maximum
            ...
            PacketSendUtility.sendPacket(player, SM_CUBE_UPDATE.stigmaSlots(player.getCommonData()
                    .getAdvancedStigmaSlotSize(), false));
            return true;
        }
        return false;
    }
    ...
    public static boolean notifyEquipAction(Player player, Item resultItem, long slot) {
        if (resultItem.getItemTemplate().isStigma()) {
            ...
            if (player.stigmaCounter > 0) {
                PacketSendUtility.sendBrightYellowMessageOnCenter(player, "Вы можете ставить стигму только раз в 15 секунд!");
                return false;
            }
            player.addStigmaTask();
            ...
            //Потрачено: %num0 SP.
            PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300411, shardCount));
            //TODO
            //Получено: %num0 SP.
            //PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300412, shardCount));
            //TODO
            //1401755
            //Нельзя поменять и использовать данную стигму. Заменять можно только стигмы одного типа.
            //--------------------------------------------------------------------------------------
            //1300444
            //Невозможно удалить кристалл стигмы: необходимо снять все снаряжение, надетое с помощью умения стигмы.
        }
        return true;
    }
