...
public class BrokerService {
    ...
    public void registerItem(Player player, int itemUniqueId, int count, long price) {
        ...
        if (itemToRegister.isGMAddItems()) {
            PacketSendUtility.sendBrightYellowMessageOnCenter(player, "Нельзя класть на склад аккаунта прописанные предметы!");
            return;
        }
        ...
        if (itemToRegister.getItemTemplate().isStackable() && count < itemToRegister.getItemCount()) {
            ...
            itemToRegister = ItemFactory.newItem(itemId, count, AddItemType.BROKER, null);
        } else {
