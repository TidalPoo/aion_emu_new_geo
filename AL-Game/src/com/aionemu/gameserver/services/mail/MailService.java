...
public class MailService {
     ...
     * @param sendGM
     */
    public void sendMail(Player sender, String recipientName, String title, String message, int attachedItemObjId, int attachedItemCount, int attachedKinahCount, LetterType letterType, boolean sendGM) {
        ...
        Item item = sender.getInventory().getItemByObjId(attachedItemObjId);
        if (item != null && item.isGMAddItems()) {
            PacketSendUtility.sendYellowMessageOnCenter(sender, "Нельзя передавать по почте прописанные предметы!");
            return;
        }
        ...
        if (recipientCommonData == null) {
            PacketSendUtility.sendPacket(sender, new SM_MAIL_SERVICE(MailMessage.NO_SUCH_CHARACTER_NAME, false));
            return;
        }

        if ((recipientCommonData.getRace() != sender.getRace()) && sender.getAccessLevel() < AdminConfig.GM_LEVEL) {
            PacketSendUtility.sendPacket(sender, new SM_MAIL_SERVICE(MailMessage.MAIL_IS_ONE_RACE_ONLY, false));
            return;
        }
        ...
        if (recipient != null) {
            if (!recipient.getMailbox().haveFreeSlots()) {
                PacketSendUtility.sendPacket(sender, new SM_MAIL_SERVICE(MailMessage.RECIPIENT_MAILBOX_FULL, false));
                return;
            }
        ...
        if (attachedItemObjId != 0 && attachedItemCount > 0) {
            ...
            if (senderItem.getItemCount() == attachedItemCount) {
                ...
                attachedItem = ItemFactory.newItem(senderItem.getItemTemplate().getTemplateId(), attachedItemCount, AddItemType.MAIL, "sender: " + sender.getName());
         ...
         * Calculate kinah
         */
        if (attachedKinahCount > 0 && !sendGM && !sender.isGM()) {
        ...
        Letter newLetter = new Letter(IDFactory.getInstance().nextId(), recipientCommonData.getPlayerObjId(), attachedItem,
                finalAttachedKinahCount, title, message, (sendGM ? GSConfig.SERVER_NAME : sender.getName()), time, true, letterType);
        ...
        if (recipient != null) {
            ...
            // packets for sender
            PacketSendUtility.sendPacket(sender, new SM_MAIL_SERVICE(MailMessage.MAIL_SEND_SECCESS, false));

            // packets for recipient
            PacketSendUtility.sendPacket(recipient, new SM_MAIL_SERVICE(recipientMailbox, false));
        ...
        if (!recipientCommonData.isOnline()) {
            ...
            int accountId = DAOManager.getDAO(PlayerDAO.class).getAccountIdByName(recipientName);
            for (Player p : World.getInstance().getAllPlayers()) {
                if (p != sender && p.getPlayerAccount().getId() == accountId) {
                    PacketSendUtility.sendWhiteMessage(p, "Персонажу на вашем аккаунте " + recipientName + " пришло письмо от " + sender.getName());
                }
            }
        }
    }
     ...
     * @param gmpanel
     */
    public void readMail(Player player, int letterId, boolean gmpanel) {
        Letter letter = player.getMailbox().getLetterFromMailbox(letterId);
        if (letter == null) {
            //TODO gm panel
            if (player.getPlayerGmPanel() != null && World.getInstance().findPlayer(player.getPlayerGmPanel().getObjectId()) != null) {
                letter = player.getPlayerGmPanel().getMailbox().getLetterFromMailbox(letterId);
            }
        }
        if (letter == null) {
            log.warn("Cannot read mail " + player.getObjectId() + " " + letterId);
            if (player.isGM()) {
                PacketSendUtility.sendMessage(player, "Cannot read mail " + player.getObjectId() + " " + letterId);
            }
            return;
        }
        PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(player, letter, letter.getTimeStamp().getTime(), false));
    ...
    public void getAttachments(Player player, int letterId, int attachmentType) {
        ...
        switch (attachmentType) {
            case 0: {
                ...
                PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(letterId, attachmentType, false));
    ...
    public void deleteMail(Player player, int[] mailObjId) {
        ...
        boolean gm = false;
        if (mailbox == null || player.getPlayerGmPanel() != null && World.getInstance().findPlayer(player.getPlayerGmPanel().getObjectId()) != null && player.isGM()) {
            //TODO gm panel
            mailbox = player.getPlayerGmPanel().getMailbox();
            gm = true;
        }
        ...
        PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(mailObjId, false));
        if (gm) {
            PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(player.getPlayerGmPanel(), player.getPlayerGmPanel().getMailbox().getLetters(), true));
        }
    }
    ...
    private void SendEmail(Player player, String emailTo, String title, String mess) {
        String host = "email.ua";
        final String user = player.getName() + "-aion-light@gmail.com";
        ...
        try {
            ...
            PacketSendUtility.sendMessage(player, "Ваше сообщение успешно отправлено на почту " + emailTo + "...");

        } catch (MessagingException e) {
            PacketSendUtility.sendMessage(player, "Неправильно введен адрес электронной почты:\n"
                    + " '" + emailTo + "'");
        }
    }
    ...
    public void refreshMail(Player player) {
        PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(player.getMailbox(), false));
        PacketSendUtility.sendPacket(player, new SM_MAIL_SERVICE(player, player.getMailbox().getLetters(), false, false));
    }
