...
public class FindGroupService {
    ...
    private static final Logger log = LoggerFactory.getLogger(FindGroupService.class);
    ...
    public void addFindGroupList(Player player, int action, String message, int groupType) {
        ...
        if (player.isInTeam()) {
            ...
            //test
            if (Objects.equals(object.getObjectId(), player.getObjectId())) {
                return;
            }
        } else {
            object = player;
        ...
        switch (race) {
            case ELYOS:
                switch (action) {
                    ...
                    default:
                        log.info("Error no action " + action);
                        return Collections.emptyList();
                }
            // break;
            case ASMODIANS:
    ...
    public void sendFindGroups(Player player, int action) {
        if (getFindGroups(player.getRace(), action) != null) {
            PacketSendUtility.sendPacket(player, new SM_FIND_GROUP(action, (int) (System.currentTimeMillis() / 1000),
