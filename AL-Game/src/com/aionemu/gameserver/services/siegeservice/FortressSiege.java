        ...
        if (legionBGeneral != 0) {
            ...
            if (legionRewards != null) {
                for (SiegeLegionReward medalsType : legionRewards) {
                    ...
                    //выдача toll легиону
                    for (Player player : LegionService.getInstance().getLegion(getSiegeLocation().getLegionId()).getOnlineLegionMembers()) {
                        if (medalsType.getToll() != 0) {
                            InGameShopEn.getInstance().addToll(player, medalsType.getToll());
                        }
                    }
    ...
    protected void giveRewardsToPlayers(SiegeRaceCounter winnerDamage) {
        ...
        for (SiegeReward topGrade : playerRewards) {
            ...
            for (int rewardedPC = 0; i < topPlayersIds.size() && rewardedPC < topGrade.getTop(); ++i) {
                ...
                //выдача toll игрокам
                if (pcd.isOnline() && topGrade.getToll() != 0) {
                    InGameShopEn.getInstance().addToll(pcd.getPlayer(), topGrade.getToll());
                }
