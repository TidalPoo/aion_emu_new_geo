...
public class InstanceService {
    ...
    public synchronized static WorldMapInstance getNextAvailableFfaInstance(IEventHolder holder) {
        int worldId = holder.getEventType().getEventTemplate().getMapId();
        int eventHandlerId = holder.getEventType().getEventTemplate().getEventId();
        ...
        int nextInstanceId = map.getNextInstanceId();
        log.info("Creating new Event instance:" + worldId + " id:" + nextInstanceId + " eventId:" + eventHandlerId);
        WorldMapInstance worldMapInstance = WorldMapInstanceFactory.createFfaWorldMapInstance(map, nextInstanceId, eventHandlerId);
     ...
     * @param instance
     */
    public static void destroyCustomInstance(WorldMapInstance instance) {
        if (instance.getEmptyInstanceTask() != null) {
            instance.getEmptyInstanceTask().cancel(false);
        }
    ...
    public static void onEnterInstance(Player player) {
        if (FfaGroupService.isInFFA(player) || FfaLegionService.isInFFA(player) || FfaPlayers.isInFFA(player) || NewPlayerZone.isInZone(player)) {
            return;
        }
    ...
    public static void onLeaveInstance(Player player) {
        if (FfaGroupService.isInFFA(player) || FfaLegionService.isInFFA(player) || FfaPlayers.isInFFA(player)) {
            return;
        }
    ...
    public static int getInstanceRate(Player player, int mapId) {
        ...
        if (player.havePermission((byte) 4)) {
            instanceCooldownRate = 2;
        }
