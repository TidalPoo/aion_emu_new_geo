...
public class DropService {
    ...
    public void requestDropItem(Player player, int npcId, int itemIndex, boolean autoLoot) {
        ...
        if (dropItems == null || dropNpc == null) {
            PacketSendUtility.sendMessage(player, "This NPC is drop empty");
            return;
        }
        ...
        if (itemId == 182400001) {
            // to do distribution
            currentDropItemCount = ItemService.addItem(player, itemId, currentDropItemCount, ItemService.DEFAULT_UPDATE_PREDICATE, AddItemType.DROP, "npcid: " + npc.getNpcId() + " player: " + player.getName() + "(" + player.getObjectId() + ")");
        ...
        if (autoLoot) {
            ...
            if (requestedItem.getWinningPlayer() != null) {
                currentDropItemCount = ItemService.addItem(requestedItem.getWinningPlayer(), itemId, currentDropItemCount,
                        new TempTradeDropPredicate(dropNpc), AddItemType.DROP, "npcid: " + npc.getNpcId() + " player: " + player.getName() + "(" + player.getObjectId() + ")");
    ...
    public void requestPlayerDropItem(Player player, int npcId, int itemIndex, boolean autoLoot) {
        Set<DropItem> dropItems = DropRegistrationService.getInstance().getCurrentDropMap().get(npcId);
        DropNpc dropNpc = DropRegistrationService.getInstance().getDropRegistrationMap().get(npcId);
        Npc npc = (Npc) World.getInstance().findVisibleObject(npcId);
        DropItem requestedItem = null;
        ...
        if (itemId == 182400001) {
            // to do distribution
            currentDropItemCount = ItemService.addItem(player, requestedItem.getDropTemplate().getItemId(), requestedItem.getCount(), ItemService.DEFAULT_UPDATE_PREDICATE, AddItemType.DROP, "npcid: " + npc.getNpcId() + " player: " + player.getName() + "(" + player.getObjectId() + ")");
        } else if (!player.isInGroup2() && !player.isInAlliance2() && !requestedItem.isItemWonNotCollected()
                && dropNpc.getDistributionId() == 0) {
            currentDropItemCount = ItemService.addItem(player, requestedItem.getItem(), ItemService.DEFAULT_UPDATE_PREDICATE, AddItemType.DROP, "npcid: " + npc.getNpcId() + " player: " + player.getName() + "(" + player.getObjectId() + ")");
            uniqueDropAnnounce(player, requestedItem);
        }
        ...
        if (autoLoot) {
            ...
            if (requestedItem.getWinningPlayer() != null) {
                currentDropItemCount = ItemService.addItem(requestedItem.getWinningPlayer(), requestedItem.getItem(),
                        new TempTradeDropPredicate(dropNpc), AddItemType.DROP, "npcid: " + npc.getNpcId() + " player: " + player.getName() + "(" + player.getObjectId() + ")");
                winningNormalActions(player, npcId, requestedItem);
                uniqueDropAnnounce(player, requestedItem);
            }
