 ...
 * @modified Alex - added equip parameter in player_initial_data and modified on
 * this service, and fix bonus title set on enter world
 */
public class PlayerService {
    ...
    public static void storePlayer(Player player) {
        ...
        DAOManager.getDAO(PlayerPunishmentsDAO.class).storePlayerPunishments(player, PunishmentType.GAG);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).storePlayerPunishments(player, PunishmentType.PRIVATESHOP);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).storePlayerPunishments(player, PunishmentType.INGAMESHOP);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).storePlayerPunishments(player, PunishmentType.TELEPORT);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).storePlayerPunishments(player, PunishmentType.MOVE);
        DAOManager.getDAO(InventoryDAO.class).store(player);
    ...
    public static Player getPlayer(int playerObjId, Account account) {
        ...
        player.setRequestFriendList(DAOManager.getDAO(RequestFriendListDAO.class).load(player));//new
        player.setRequestSenderList(DAOManager.getDAO(RequestFriendListDAO.class).loadSender(player));//new
        ...
        DAOManager.getDAO(PlayerPunishmentsDAO.class).loadPlayerPunishments(player, PunishmentType.GAG);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).loadPlayerPunishments(player, PunishmentType.PRIVATESHOP);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).loadPlayerPunishments(player, PunishmentType.INGAMESHOP);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).loadPlayerPunishments(player, PunishmentType.TELEPORT);
        DAOManager.getDAO(PlayerPunishmentsDAO.class).loadPlayerPunishments(player, PunishmentType.MOVE);
        ...
        // load event items max count of day
    ...
    public static Player newPlayer(PlayerCommonData playerCommonData, PlayerAppearance playerAppearance, Account account) {
        ...
        if (account.getAccessLevel() > 0) {
            ...
            Item item = ItemFactory.newItem(itemId, 1, AddItemType.GAMEMASTER, GSConfig.SERVER_NAME);
            ...
            newPlayer.setBanItems(true);
        } else if (playerCreationData != null) { // player transfer
            List<ItemType> items = playerCreationData.getItems();
            for (ItemType itemType : items) {
                int itemId = itemType.getTemplate().getTemplateId();
                Item item = ItemFactory.newItem(itemId, itemType.getCount(), AddItemType.GAMEMASTER, GSConfig.SERVER_NAME);
                if (item == null) {
                    continue;
                }

                if (itemType.getRace() != Race.PC_ALL && playerCommonData.getRace() != itemType.getRace()) {
                    continue;
                }
                item.setAddTime(new Timestamp(System.currentTimeMillis()));
                if (itemType.getEnchantLevel() != 0) {
                    item.setEnchantLevel(itemType.getEnchantLevel());
                }
                ...
                ItemTemplate itemTemplate = item.getItemTemplate();
                if (itemType.isEquip() && (itemTemplate.isArmor() || itemTemplate.isWeapon())) {
                    item.setEquipped(true);
                    ItemSlot itemSlot = ItemSlot.getSlotFor(itemTemplate.getItemSlot());

                    if (itemSlot.getSlotIdMask() == ItemSlot.EARRINGS_LEFT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.EARRINGS_LEFT.getSlotIdMask())) {
                        itemSlot = ItemSlot.EARRINGS_RIGHT;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.EARRINGS_RIGHT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.EARRINGS_RIGHT.getSlotIdMask())) {
                        itemSlot = ItemSlot.EARRINGS_LEFT;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.RING_LEFT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.RING_LEFT.getSlotIdMask())) {
                        itemSlot = ItemSlot.RING_RIGHT;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.RING_RIGHT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.RING_RIGHT.getSlotIdMask())) {
                        itemSlot = ItemSlot.RING_LEFT;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.MAIN_HAND.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.MAIN_HAND.getSlotIdMask())) {
                        itemSlot = ItemSlot.SUB_HAND;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.POWER_SHARD_LEFT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.POWER_SHARD_LEFT.getSlotIdMask())) {
                        itemSlot = ItemSlot.POWER_SHARD_RIGHT;
                    } else if (itemSlot.getSlotIdMask() == ItemSlot.POWER_SHARD_RIGHT.getSlotIdMask() && equipment.isSlotEquipped(ItemSlot.POWER_SHARD_RIGHT.getSlotIdMask())) {
                        itemSlot = ItemSlot.POWER_SHARD_LEFT;
                    }
                    if (itemSlot.getSlotIdMask() == ItemSlot.POWER_SHARD_RIGHT.getSlotIdMask()) {
                        newPlayer.getCommonData().setPowerShard(1);
                    }

                    //no send to account
                    item.setAddItems(AddItemType.STARTEQUIP.getType());
                    item.setEquipmentSlot(itemSlot.getSlotIdMask());
                    equipment.onLoadHandler(item);
                } else {
