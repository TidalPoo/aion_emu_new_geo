...
public final class QuestService {
        ...
        if (dialogId != DialogAction.SELECTED_QUEST_NOREWARD.id() && dialogId != 0 && !extended) {
            ...
            if (isLastRepeat && template.isUseSingleClassReward() || template.isUseRepeatedClassReward()) {
                ...
                if (dialogId - 8 >= 0 && dialogId - 8 < rewards.getSelectableRewardItem().size()) {
                    ...
                    log.error("The SelectableRewardItem list has no element with the given index (dialogId - 8) of " + (dialogId - 8)
                            + ". See quest id " + env.getQuestId());
                }
    ...
    private static boolean checkStartConditionsImpl(QuestEnv env, boolean warn) {
        ...
        int requiredStartConditions = template.getXMLStartConditions().size();
        if (requiredStartConditions > 0 && template.isProfession()) {
            requiredStartConditions += template.isMaster() ? 1 - CraftConfig.MAX_MASTER_CRAFTING_SKILLS
                    : 2 - CraftConfig.MAX_EXPERT_CRAFTING_SKILLS;
        }
