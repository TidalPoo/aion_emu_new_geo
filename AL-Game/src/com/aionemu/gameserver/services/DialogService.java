...
public class DialogService {
    ...
    public static void onDialogSelect(int dialogId, final Player player, Npc npc, int questId, int extendedRewardIndex) {
        ...
        if (CustomConfig.ENABLE_SHOW_DIALOGID && player.isGM()) {
            PacketSendUtility.sendMessage(player, "dialogId: " + dialogId);
            PacketSendUtility.sendMessage(player, "questId: " + questId);
            if (DialogAction.getActionByDialogId(dialogId) != null) {
                PacketSendUtility.sendMessage(player, "switch name: " + DialogAction.getActionByDialogId(dialogId).name());
            }
        }
        ...
        if (questId == 0) {
            switch (DialogAction.getActionByDialogId(dialogId)) {
                case BUY: {
                    ...
                    if (player.getWorldId() != WorldMapType.SARPAN.getId() && player.getWorldId() != WorldMapType.SARPAN_SKY.getId()) {
                        if (npc.getRace().getRaceId() < 2 && !player.getRace().equals(npc.getRace())) {
                            PacketSendUtility.sendMessage(player, "Your race does not coincide with the race NPCs!!");
                            break;
                        }
                    }
                ...
                case EDIT_GENDER: { // (2.5)
                    boolean have_edit = player.havePermission((byte) MembershipConfig.EDIT_GENDER);
                    boolean have_character = player.havePermission((byte) MembershipConfig.EDIT_CHARACTER);
                    ...
                    if (dialogId == DialogAction.EDIT_GENDER.id()) {
                        ...
                        if (player.getInventory().getItemCountByItemId(169660000) > 0 || player.getInventory().getItemCountByItemId(169660001) > 0
                                || have_edit || player.getAccessLevel() > 0 || player.isDeveloper()) {
                            check_ticket = 1;
                        }
                    } else {
                        // Plastic Surgery
                        if (player.getInventory().getItemCountByItemId(169650000) > 0 || player.getInventory().getItemCountByItemId(169650001) > 0
                                || have_character || player.getAccessLevel() > 0 || player.isDeveloper()) {
                            check_ticket = 1;
                        }
                    }
                ...
                case TRADE_SELL_LIST: {
                    ...
                    int sellModificator = TradeService.getSellModificator(npc.getNpcId());
                    PacketSendUtility.sendPacket(player, new SM_SELL_ITEM(targetObjectId, tradeListTemplate, sellModificator));
                    break;
                }
