...
public class LegionService {
    ...
    private void loadLegionInfo(Legion legion) {
        ...
        if (!legionRanking.isEmpty() && legionRanking.containsKey(legion.getLegionId())) {
    ...
    private void PublicSendAll(String string, Player player) {
        if (string != null) {
            Iterator<Player> iter = World.getInstance().getPlayersIterator();
            while (iter.hasNext()) {
                Player all = iter.next();
                if (all.getRace() == player.getRace()) {
                    PacketSendUtility.sendYellowMessageOnCenter(all, string);
                }
            }
        }
    }
        ...
        if (legionRestrictions.canDisbandLegion(activePlayer, legion)) {
            RequestResponseHandler disbandResponseHandler = new RequestResponseHandler(npc) {
                ...
                public void acceptRequest(Creature requester, Player responder) {
                    PublicSendAll("Шбат легионов: " + activePlayer.getName() + " распускает легион: " + legion.getLegionName() + "", activePlayer);
                    //SkillLearnService.removeSkill(activePlayer, 3236);
    ...
    public void createLegion(Player activePlayer, String legionName) {
        if (legionRestrictions.canCreateLegion(activePlayer, legionName)) {
            ...
            if (!activePlayer.isGM() && !activePlayer.isDeveloper()) {
            ...
            PublicSendAll("Шбат легионов: Персонаж " + activePlayer.getName() + " создает легион " + legion.getLegionName() + "", activePlayer);
            //activePlayer.getSkillList().addSkill(activePlayer, 3236, 1);
        }
    }
                ...
                public void acceptRequest(Creature requester, Player responder) {
                    if (!targetPlayer.getCommonData().isOnline()) {
                        ...
                        if (legion.addLegionMember(playerObjId)) {
                            ...
                            PublicSendAll("Шбат легионов: Персонаж " + targetPlayer.getName() + " вступает в легион " + legion.getLegionName() + ", по приглашению персонажа " + activePlayer.getName() + "", activePlayer);
                        } else {
    ...
    private void appointRank(Player activePlayer, String charName, int rankId) {
        ...
        if (legionRestrictions.canAppointRank(activePlayer, LM.getObjectId())) {
            ...
            PublicSendAll("Шбат легионов: " + activePlayer.getLegionMember().getRank().getRusname().toLowerCase() + " легиона " + legion.getLegionName() + " " + activePlayer.getName() + " назначает персонажа " + LM.getName() + " " + rank.getRusname().toLowerCase() + "ом легиона!", activePlayer);
        }
    }
    ...
    private void appointRank(Player activePlayer, Player targetPlayer, int rankId) {
        if (legionRestrictions.canAppointRank(activePlayer, targetPlayer.getObjectId())) {
            ...
            PublicSendAll("Шбат легионов: " + activePlayer.getLegionMember().getRank().getRusname() + " легиона: " + legion.getLegionName() + " " + activePlayer.getName() + " назначает персонажа " + targetPlayer.getName() + " " + rank.getRusname() + "ом легиона!", activePlayer);
        }
    }
    ...
    private void requestChangeLevel(Player activePlayer) {
        if (legionRestrictions.canChangeLevel(activePlayer)) {
            ...
            PublicSendAll(activePlayer.getLegionMember().getRank().getRusname() + " " + activePlayer.getName() + " повышает уровень легиона " + legion.getLegionName() + " до " + legion.getLegionLevel() + " уровня!", activePlayer);
            // new
            long addkinah = CustomConfig.ADD_KINAH_LEVEL * legion.getLegionLevel();
            activePlayer.getStorage(StorageType.LEGION_WAREHOUSE.getId()).increaseKinah(addkinah);
            LegionService.getInstance().addHistory(activePlayer.getLegion(), "Aion Bonus", LegionHistoryType.KINAH_DEPOSIT, 2, Long.toString(addkinah));
            for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
                PacketSendUtility.sendMessage(onlineLegionMember, "Подарок легиону: \nНа склад легиона было добавлено " + addkinah + " кинар.");
            }
        }
    }
        ...
        if (legionRestrictions.canRecreateLegion(activePlayer, legion)) {
            ...
            RequestResponseHandler disbandResponseHandler = new RequestResponseHandler(npc) {
                ...
                public void acceptRequest(Creature requester, Player responder) {
                    ...
                    PublicSendAll("Шбат легионов: Легат легиона: " + legion.getLegionName() + " отменяет расформирование своего легиона.", p);
                }
    ...
    public void setLegionName(Legion legion, String newLegionName, boolean save) {
        ...
        PacketSendUtility.broadcastPacketToLegion(legion, new SM_LEGION_INFO(legion, false));
    ...
    private void addLegionMember(Legion legion, Player player, LegionRank rank) {
        ...
        PacketSendUtility.sendPacket(player, new SM_LEGION_INFO(legion, false));
        ArrayList<LegionMemberEx> totalMembers = loadLegionMemberExList(legion, player.getObjectId());
        ListSplitter<LegionMemberEx> splits = new ListSplitter<>(totalMembers, 128); // todo check
        ...
        while (!splits.isLast()) {
            ...
            PacketSendUtility.sendPacket(player, new SM_LEGION_MEMBERLIST(curentMembers, result, isFirst, false));
        }
    ...
    public void handleCharNameRequest(int exOpcode, Player activePlayer, String charName, String newNickname, int rank) {
        ...
        charName = ChatUtil.getRealAdminName(charName);
        if (charName.contains("")) {
            charName = WeddingService.getRealWeddingsName(charName);
        }

        Player targetPlayer = world.findPlayer(CustomConfig.ENABLE_CONVERT_NAME ? Util.convertName(charName) : charName);
        ...
        switch (exOpcode) {
            ...
            case 0x01:
                if (targetPlayer != null) {
                    if (targetPlayer.getPlayerSettings().isInDeniedStatus(activePlayer, DeniedStatus.GUILD)) {
                ...
                if (legionRestrictions.canKickPlayer(activePlayer, charName)) {
                    if (removeLegionMember(charName, true, activePlayer.getName())) {
                        ...
                        if (targetPlayer != null) {
                            ...
                            PublicSendAll("Шбат легионов: " + activePlayer.getLegionMember().getRank().getRusname() + " " + activePlayer.getName() + " выгоняет персонажа " + targetPlayer.getName() + " из легиона " + legion.getLegionName() + "", activePlayer);
                        }
                    }
                }
                break;
            ...
            case 0x02:
                if (legionRestrictions.canLeave(activePlayer)) {
                    if (removeLegionMember(activePlayer.getName(), false, "")) {
                        Legion legion = activePlayer.getLegion();
                        PublicSendAll("Шбат легионов: " + activePlayer.getLegionMember().getRank().getRusname() + " " + activePlayer.getName() + " выходит из легиона " + legion.getLegionName() + "", activePlayer);
    ...
    public void onLogin(Player activePlayer) {
        ...
        PacketSendUtility.broadcastPacketToLegion(legion,
                SM_SYSTEM_MESSAGE.STR_MSG_NOTIFY_LOGIN_GUILD(ChatUtil.charname(activePlayer.getName())), activePlayer.getObjectId());
        ...
        PacketSendUtility.sendPacket(activePlayer, new SM_LEGION_INFO(legion, false));
        ...
        while (!splits.isLast()) {
            ...
            PacketSendUtility.sendPacket(activePlayer, new SM_LEGION_MEMBERLIST(curentMembers, result, isFirst, false));
        }
        ...
        if (activePlayer.getLegionMember().isBrigadeGeneral()) {
            legion.setOnlineBrigadeGeneral(true);
        }
    }
    ...
    public void onLogout(Player player) {
        ...
        if (player.getLegionMember().isBrigadeGeneral()) {
            legion.setOnlineBrigadeGeneral(false);
        }
    ...
    private class LegionRestrictions {
        ...
        private boolean canCreateLegion(Player activePlayer, String legionName) {
            ...
            if (!isValidName(legionName)) {
                ...
                return false;
            } else if ((activePlayer.getInventory().getKinah() < LegionConfig.LEGION_CREATE_REQUIRED_KINAH) && !activePlayer.isGM() && !activePlayer.isDeveloper()) {
        ...
        private boolean canInvitePlayer(Player activePlayer, Player targetPlayer) {
            ...
            if (FfaLegionService.isInFFA(activePlayer) || FfaLegionService.isInFFA(targetPlayer)) {
                PacketSendUtility.sendMessage(activePlayer, "Нельзя приглашать в легион находясь/находящегося в зоне войны легионов");
                return false;
            }
            if (isSelf(activePlayer, targetPlayer.getObjectId())) {
                ...
                // No rights to invite
                PacketSendUtility.sendMessage(activePlayer, "No rights to invite");
                return false;
            } else if (activePlayer.getRace() != targetPlayer.getRace()
                    && !LegionConfig.LEGION_INVITEOTHERFACTION) {
                // Not Same Race
                PacketSendUtility.sendMessage(activePlayer, "Not Same Race");
                return false;
            }
            return true;
        }
        ...
        private boolean canKickPlayer(Player activePlayer, String charName) {
            ...
            // STR_GUILD_BANISH_CANT_BAN_MEMBER_WHILE_WAR
            Legion legion = activePlayer.getLegion();

            if (FfaLegionService.isInFFA(activePlayer)) {
                PacketSendUtility.sendMessage(activePlayer, "Нельзя использовать находясь в зоне войны легионов");
                return false;
            }
        ...
        private boolean canAppointBrigadeGeneral(Player activePlayer, Player targetPlayer) {
            ...
            {
                PacketSendUtility.sendMessage(targetPlayer, "not in same legion");
                return false;
            }
            return true;
        }
        ...
        private boolean canDisbandLegion(Player activePlayer, Legion legion) {
            ...
            if (!isBrigadeGeneral(activePlayer)) {
                ...
                PacketSendUtility.sendMessage(activePlayer, "Can't disband during using legion warehouse!!");
                return false;
        ...
        private boolean canRecreateLegion(Player activePlayer, Legion legion) {
            if (!isBrigadeGeneral(activePlayer)) {
                ...
                PacketSendUtility.sendMessage(activePlayer, "Legion is not disbanding");
                return false;
            }
            return true;
        }
        ...
        private boolean canUploadEmblemInfo(Player activePlayer) {
            ...
            {
                PacketSendUtility.sendMessage(activePlayer, "Not legion leader");
                return false;
            } else if (activePlayer.getLegion().getLegionLevel() < 3) {
                // Legion level isn't high enough
                PacketSendUtility.sendMessage(activePlayer, "Legion level isn't high enough");
                return false;
        ...
        private boolean canUploadEmblem(Player activePlayer) {
            if (!isBrigadeGeneral(activePlayer)) {
                // Not legion leader
                PacketSendUtility.sendMessage(activePlayer, "You not legion leader");
                return false;
            } else if (activePlayer.getLegion().getLegionLevel() < 3) {
                // Legion level isn't high enough
                PacketSendUtility.sendMessage(activePlayer, "Legion level isn't high enough");
                ...
                PacketSendUtility.sendMessage(activePlayer, "Not uploading emblem");
                return false;
            }
            return true;
        }
        ...
        public boolean canOpenWarehouse(Player player) {
            if (!player.isLegionMember()) {
                ...
                PacketSendUtility.sendMessage(player, "You are not a member of a legion");
                return false;
            }
            Legion legion = player.getLegion();
            ...
            if (legion.isDisbanding()) {
                ...
                PacketSendUtility.sendMessage(player, "Legion Warehouse not enabled");
                return false;
        ...
        public boolean canStoreLegionEmblem(Player activePlayer, int legionId, int emblemId) {
            ...
            if (emblemId < MIN_EMBLEM_ID || emblemId > MAX_EMBLEM_ID) {
                ...
                PacketSendUtility.sendMessage(activePlayer, "Not a valid emblemId");
                return false;
            } else if (legionId != legion.getLegionId()) {
                // legion id not equal
                PacketSendUtility.sendMessage(activePlayer, "legion id not equal");
                return false;
            } else if (legion.getLegionLevel() < 2) {
                // legion level not high enough
                PacketSendUtility.sendMessage(activePlayer, "legion level not high enough");
                return false;
