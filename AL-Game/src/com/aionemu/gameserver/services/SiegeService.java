...
public class SiegeService {
   ...
   // private static final SiegeService instance = new SiegeService();
    ...
    private final FastMap<Integer, VisibleObject> moltenusAbyssBoss = new FastMap<>();
    ...
    public void initSiegeLocations() {
        if (SiegeConfig.SIEGE_ENABLED) {
            ...
            log.info("Loaded " + sources.size() + " Source");
            log.info("Loaded " + fortresses.size() + " Fortess");
            log.info("Loaded " + outposts.size() + " Outposts");
            log.info("Loaded " + artifacts.size() + " Artifacts");
                ...
                if (moltenusAbyssBoss.containsKey(251045) && moltenusAbyssBoss.get(251045).isSpawned()) {
                    ...
                    World.getInstance().doOnAllPlayers(new Visitor<Player>() {
                        ...
                        public void visit(Player player) {
                            PacketSendUtility.sendBrightYellowMessage(player, "Менотиос: Фрагмент Гнева был замечен в Бездне!");
                        }
                        ...
                        public void run() {
                            for (VisibleObject vo : moltenusAbyssBoss.values()) {
                                ...
                                World.getInstance().doOnAllPlayers(new Visitor<Player>() {
                                    ...
                                    @Override
                                    public void visit(Player player) {
                                        PacketSendUtility.sendBrightYellowMessage(player, "Менотиос: Фрагмент Гнева покинул Бездну!");
                                    }
    ...
    public void fortressBuffApply(Player player) {
        if (player.getWorldId() == 600050000 || player.getWorldId() == 600060000 || player.getWorldId() == 400010000 || player.getWorldId() == 210050000 || player.getWorldId() == 220070000) {
            ...
            SiegeLocation location1131 = getSiegeLocation(1131);
            SiegeLocation location1132 = getSiegeLocation(1132);
            SiegeLocation location1141 = getSiegeLocation(1141);
            SiegeLocation location1221 = getSiegeLocation(1221);
            SiegeLocation location1231 = getSiegeLocation(1231);
            SiegeLocation location1241 = getSiegeLocation(1241);
            SiegeLocation location1251 = getSiegeLocation(1251);
            SiegeLocation location2021 = getSiegeLocation(2021);
            SiegeLocation location3011 = getSiegeLocation(3011);
            SiegeLocation location3021 = getSiegeLocation(3021);
            if (player.getWorldId() == 600050000) {
                ...
                } else if (location5011.getRace() != SiegeRace.getByRace(player.getRace())) {
            ...
            } else if (player.getWorldId() == 400010000) {
                if (location1131.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Западная сиэль
                    SkillEngine.getInstance().applyEffectDirectly(12070, player, player, 0);
                }

                if (location1132.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Восточная сиэль
                    SkillEngine.getInstance().applyEffectDirectly(12071, player, player, 0);
                }

                if (location1141.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Крепость серного дерева
                    SkillEngine.getInstance().applyEffectDirectly(12072, player, player, 0);
                }

                if (location1221.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Кротан
                    SkillEngine.getInstance().applyEffectDirectly(12075, player, player, 0);
                }

                if (location1231.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Ткисас
                    SkillEngine.getInstance().applyEffectDirectly(12076, player, player, 0);
                }

                if (location1241.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Ра-Мирен
                    SkillEngine.getInstance().applyEffectDirectly(12077, player, player, 0);
                }

                if (location1251.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Астерия
                    SkillEngine.getInstance().applyEffectDirectly(12074, player, player, 0);
                }
            } else if (player.getWorldId() == 210050000) {
                if (location2021.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Алтарь алчности
                    SkillEngine.getInstance().applyEffectDirectly(12106, player, player, 0);
                }
            } else if (player.getWorldId() == 220070000) {
                if (location3011.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Запечатанная башня
                    SkillEngine.getInstance().applyEffectDirectly(12109, player, player, 0);
                }

                if (location3021.getRace() == SiegeRace.getByRace(player.getRace())) {
                    //Храм красной земли
                    SkillEngine.getInstance().applyEffectDirectly(12112, player, player, 0);
                }
            }
        }
    }
    ...
    public void fortressBuffRemove(Player player) {
        ...
        } else if (player.getEffectController().hasAbnormalEffect(12070)) {
            player.getEffectController().removeEffect(12070);
        } else if (player.getEffectController().hasAbnormalEffect(12071)) {
            player.getEffectController().removeEffect(12071);
        } else if (player.getEffectController().hasAbnormalEffect(12072)) {
            player.getEffectController().removeEffect(12072);
        } else if (player.getEffectController().hasAbnormalEffect(12075)) {
            player.getEffectController().removeEffect(12075);
        } else if (player.getEffectController().hasAbnormalEffect(12076)) {
            player.getEffectController().removeEffect(12076);
        } else if (player.getEffectController().hasAbnormalEffect(12077)) {
            player.getEffectController().removeEffect(12077);
        } else if (player.getEffectController().hasAbnormalEffect(12074)) {
            player.getEffectController().removeEffect(12074);
        } else if (player.getEffectController().hasAbnormalEffect(12106)) {
            player.getEffectController().removeEffect(12106);
        } else if (player.getEffectController().hasAbnormalEffect(12109)) {
            player.getEffectController().removeEffect(12109);
        } else if (player.getEffectController().hasAbnormalEffect(12112)) {
            player.getEffectController().removeEffect(12112);
    ...
    public void onWeatherChanged(WeatherEntry entry) {
        /*
         int siegeLocationId = 10000;
         Collection<SiegeNpc> siegeNpcs = World.getInstance().getLocalSiegeNpcs(siegeLocationId);
         for (SiegeNpc npc : siegeNpcs) {
         Boolean condition = npc.getObjectTemplate().getMistSpawnCondition();
         if (condition == null)
         continue;
         if (npc.isInsideWeatherZone(entry.getZoneId())) {
         if (entry.getCode() == 3 && condition || entry.getCode() == 2 && !condition)
         npc.getController().onDelete();
         }
         }
         */
