...
public class AbyssPointsService {
    ...
    public static void addAp(Player player, VisibleObject obj, int value) {
        ...
        if (!(obj instanceof Player) && value > 5000){
            value = 150;
        }
        // if (player.getAbyssRank().getAp())
        //AbyssPointData apdata = DataManager.abyssPointData;
        //if (apdata.getTelelocationTemplate(player.getLevel()).getMaxPoint() )
        addAp(player, value);
    }
    ...
    public static void setAp(Player player, int value) {
        if (player == null || player.getAccessLevel() != 0 && player.getAccessLevel() <= AdminConfig.GM_NO_ABYSS_POINTS) {
            return;
        }
