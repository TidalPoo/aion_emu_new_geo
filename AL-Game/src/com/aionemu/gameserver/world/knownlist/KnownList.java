...
public class KnownList {
        ...
        if (knownObjects.put(object.getObjectId(), object) == null) {
            if (object instanceof Player) {
                ...
                if (owner instanceof Player) {
                    if (((Player) owner).getRace() != ((Player) object).getRace()) {
                        //TODO distance message
                        PacketSendUtility.sendMessage((Player) owner, "" + ColorUtil.convertTextToColor("Обнаружен противник", "255 0 0") + "");
                    }
                }
            }
    ...
    public void addVisualObject(VisibleObject object) {
        if (object instanceof Creature) {
            if (SecurityConfig.INVIS && object instanceof Player) {
                if (!owner.canSee((Creature) object)) {
                    return;
                }
            }
    ...
    protected boolean checkObjectInRange(VisibleObject newObject) {
        /* if (Math.abs(owner.getZ() - newObject.getZ()) < 30) {
         //return false;
         if (owner instanceof Player && newObject instanceof Player) {
         if (((Player) owner).getRace() != ((Player) newObject).getRace()) {
         //TODO distance message
         PacketSendUtility.sendMessage((Player) owner, ColorUtil.convertTextToColor("Обнаружен противник", "255 0 0"));
         }
         }
