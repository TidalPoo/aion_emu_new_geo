...
public class World {
    ...
    private final FastMap<Integer, Npc> allNpcs2;
    ...
    private World() {
        ...
        allNpcs2 = new FastMap<Integer, Npc>().shared();
    ...
    public void storeObject(VisibleObject object) {
        ...
        if (object instanceof Npc) {
            ...
            allNpcs2.put(object.getObjectId(), (Npc) object);
        }
    }
    ...
    public Collection<Npc> getNpcs2() {
        return allNpcs2.values();
    }
    ...
    public void updatePosition(VisibleObject object, float newX, float newY, float newZ, byte newHeading, boolean updateKnownList) {
        ...
        WorldPosition pos = object.getPosition();
        if (oldRegion == null) {
        ...
        if (object instanceof Player) {
            /*String text = ColorUtil.convertTextToColor("WARNING", ColorEnum.getFor("красный").getRgb());
             if (GeoDataConfig.GEO_ENABLE) {
             float z = GeoService.getInstance().getZ(object.getWorldId(), object.getX(), object.getY());
             if (object.getZ() - z > 100) {
             PacketSendUtility.sendWhiteMessageOnCenter(((Player) object), text);
             }
             }*/
            if (newZ < 0) {
                Player p = ((Player) object);
                TeleportService2.teleportTo(p, p.getSaveWorld(), p.getSaveInstance(), p.getSaveX(), p.getSaveY(), p.getSaveZ());
                return;
            }
        }
        ...
        if (newRegion == null) {
            if (object instanceof Player) {
                if (pos == null) {
                    TeleportService2.teleportToCapital((Player) object);
                } else {
                    TeleportService2.teleportTo(((Player) object), pos);
                }
                return;
            }
        ...
        if (object instanceof Player) {
            Player player = (Player) object;
            player.getMoveController().setMoveTime(System.currentTimeMillis());
        }
    }
    ...
    public void preSpawn(VisibleObject object) {
        ((Creature) object).setState(CreatureState.ACTIVE);
