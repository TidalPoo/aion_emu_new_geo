...
public class GameTime implements Cloneable {
    ...
    public void calculateDayTime() {
        int hour = getHour();
        if (hour > 21 || hour < 4) {
            dayTime = DayTime.NIGHT;// ночь
        } else if (hour > 16) {
            dayTime = DayTime.EVENING;// вечер
        } else if (hour > 8) {
            dayTime = DayTime.AFTERNOON;// полдень
        } else {
            dayTime = DayTime.MORNING;// утро
        }
    }
    ...
    public int getMonth() {
        ...
        for (Monthes m : Monthes.values()) {
            if ((minutesInYear - getProperMinutesInMonth(m)) > 0) {
                minutesInYear -= getProperMinutesInMonth(m);
                answer += 1;
