...
public abstract class AdminCommand extends ChatCommand {
    ...
    public boolean checkLevel(Player player) {
        return player.getAccessLevel() >= getLevel() || player.isDeveloper();
    }
    ...
    boolean process(Player player, String text) {
        ...
        if (!checkLevel(player)) {
            ...
            PacketSendUtility.sendMessage(player, "[WARN] You dont have enough rights to execute command " + getAlias());
            return true;
        }
