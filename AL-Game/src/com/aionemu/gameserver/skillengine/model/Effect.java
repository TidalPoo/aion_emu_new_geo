...
public class Effect implements StatOwner {
    ...
    public synchronized void endEffect() {
        ...
        if (effector instanceof Npc) {
            ((AbstractAI) effector.getAi2()).endEffect(getSkillId());
        }
    }
    ...
    public int getEffectsDuration() {
        ...
        while (itr.hasNext() && duration2 == 0) {
            ...
            duration2 = duration2 > effectDuration ? duration2 : effectDuration;
        }

        //adjust with BOOST_DURATION
        switch (skillTemplate.getSubType()) {
            case BUFF:
                duration2 = effector.getGameStats().getStat(StatEnum.BOOST_DURATION_BUFF, duration2).getCurrent();
                break;
            default:
                break;
        }
        ...
        if (effected instanceof Player && skillTemplate.getPvpDuration() != 0) {
            duration2 = duration2 * skillTemplate.getPvpDuration() / 100;
        }

        if (duration2 > 86400000) {
            duration2 = 86400000;
        }
