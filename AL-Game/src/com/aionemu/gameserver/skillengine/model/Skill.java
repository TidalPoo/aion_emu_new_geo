...
/**
 * @author ATracer Modified by Wakzashi, Alex
 */
public class Skill {
    ...
    public boolean canUseSkill() {
        ...
        if (effector instanceof Player) {
            ...
            if (skillMethod == SkillMethod.ITEM && duration > 0 && player.getMoveController().isInMove()) {
                ...
                return false;
            } else if (skillMethod == SkillMethod.ITEM && duration > 0 && getItemTemplate().getReturnAlias() != null && getItemTemplate().getReturnWorldId() != 0 && !RestrictionsManager.canTeleport(player)) {
                return false;
            }
        }
    ...
    private boolean useSkill(boolean checkAnimation, boolean checkproperties) {
        ...
        if (SecurityConfig.MOTION_TIME) {
            ...
            if (checkAnimation && !checkAnimationTime()) {
                log.debug("PlayerObjectId: " + effector.getObjectId() + " check animation time failed");
                if (!((Player) effector).isGM()) {
                    PunishmentService.setIsInPrison(((Player) effector), true, 5000, "NoAnim");
                }
                return false;
            }
        }
        ...
        if (effector instanceof Player) {
            ...
            switch (this.skillTemplate.getSkillId()) {
                case 3512:
                    if (((Player) effector).getBindPoint() == null) {
                        PacketSendUtility.sendPacket((Player) effector, new SM_SYSTEM_MESSAGE(((Player) effector).getRace() == Race.ELYOS ? 1401735 : 1401736));
                    }
                    break;
                default:
                    break;
        ...
        if (skillMethod == SkillMethod.CAST || skillMethod == SkillMethod.ITEM || skillMethod == SkillMethod.CHARGE) {
            ...
            if (effector instanceof Npc) {
                ((AbstractAI) effector.getAi2()).setSubStateIfNot(AISubState.CAST);
            }
        }
    ...
    protected void calculateSkillDuration() {
        ...
        //fix
        //skillTemplate.isBoostCastingTime() && skillTemplate.getType() == SkillType.MAGICAL
    ...
    private boolean checkAnimationTime() {
        ...
        float serverTimes = motionTime.getTimeForWeapon(player.getRace(), player.getGender(), weapons);
        int clientTime = hitTime;

        if (serverTimes == 0) {
            //log.warn("missing weapon time for motionName: " + motion.getName() + " weapons: " + weapons.toString()
            //+ " Race: " + player.getRace() + " Gender: " + player.getGender() + " skillId: " + this.getSkillId());
            return true;
        }
        ...
        if (attackSpeed.getBase() != attackSpeed.getCurrent()) {
            serverTimes *= (attackSpeed.getCurrent() / attackSpeed.getBase());
        }
        ...
        int finalTime = Math.round(serverTimes);
        if (motion.getInstantSkill() && hitTime == 0) {
    ...
    protected void endCast() {
        ...
        //todo if distance > 4 ?
        if (properties != null && effector.getTarget() != null && properties.getFirstTargetRange() > 2 && !MathUtil.isInAttackRange(effector, firstTarget, properties.getFirstTargetRange())) {
            if (effector instanceof Player) {
        ...
        /**
         * удаление итема после использования если был каст "использование
         * предмета" try removing item, if its not possible return to prevent
         * exploits
         */
        ...
        if (effector instanceof Npc) {
            ...
            ((AbstractAI) effector.getAi2()).ownerSkillUse(skillTemplate.getSkillId());
        }

        if (effector instanceof Player) {
            for (Creature nextEffected : effectedList) {
                if (nextEffected instanceof Npc) {
                    ((AbstractAI) nextEffected.getAi2()).playerSkillUse((Player) effector, skillTemplate.getSkillId());
                }
            }
            if (skillTemplate.getSkillId() == 1463) {
                //костыль ибо скилл меня заебал :)
                Player player = (Player) effector;
                if (player.getOldTarget() != null) {
                    player.setTarget(player.getOldTarget());
