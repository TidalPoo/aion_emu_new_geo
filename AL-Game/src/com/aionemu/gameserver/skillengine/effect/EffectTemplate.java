...
public abstract class EffectTemplate {
     ...
     * cannotmiss 7) check magic resist / dodge 8) addsuccess
     * <p/>
     * exceptions: buffbind buffsilence buffsleep buffstun randommoveloc
     * recallinstant returneffect returnpoint shieldeffect signeteffect
    ...
    public boolean calculate(Effect effect, StatEnum statEnum, SpellStatus spellStatus) {
        ...
        if (effect.getSkillTemplate().getSkillId() == 2806) {
            //effect.getEffected().getEffectController().isAbnormalPresentBySkillId(287) ||
            if (effect.getEffected().getTransformModel().getType() == TransformType.AVATAR && this instanceof StunEffect) {
                //Skip this by stun.
                this.addSuccessEffect(effect, spellStatus);
                return true;
            }
        ...
        /*if(effect.getEffector() instanceof Player)
         ...
         if(player.skilluse > 0 && this instanceof StunEffect){
         player.skilluse = 0;
         PacketSendUtility.sendMessage(player, "EFFECTTEMPLATE.JAVA. skilluse = 0 .");
         //Skip this by stun.
         PacketSendUtility.sendMessage(player, "TEST GOOOD");
         this.addSuccessEffect(effect, spellStatus);
         return true;
         }
         }*/
    ...
    private boolean effectConditionsCheck(Effect effect) {
        Conditions effectConditions2 = getEffectConditions();
        return effectConditions2 != null ? effectConditions2.validate(effect) : true;
    }
    ...
    public void SetDuration1(int d) {
        this.duration1 = d;
    }

    public void SetDuration2(int d) {
        this.duration2 = d;
    }
