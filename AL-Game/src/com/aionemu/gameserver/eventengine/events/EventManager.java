...
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_1x1;
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_2x2;
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_3x3;
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_4x4;
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_6x6;
import static com.aionemu.gameserver.eventengine.events.enums.EventType.E_LHE;
import com.aionemu.gameserver.eventengine.events.holders.IEventHolder;
import com.aionemu.gameserver.eventengine.events.holders.PlayerGroupEventHolder;
import com.aionemu.gameserver.eventengine.events.holders.SimpleSinglePlayerEventHolder;
import com.aionemu.gameserver.eventengine.events.holders.SinglePlayerHolder;
import com.aionemu.gameserver.eventengine.events.holders._1x1SamePlayerClassEventHolder;
import com.aionemu.gameserver.eventengine.events.xml.EventStartPosition;
import com.aionemu.gameserver.eventengine.events.xml.EventStartPositionList;
import com.aionemu.gameserver.eventengine.utils.TimeUtil;
...
public class EventManager {
    ...
    private final Map<EventType, Map<Integer, PlayerInfo>> playedPlayers;
    private final List<IEventHolder> holders;
    private final Map<EventType, Boolean> events;
    private final Map<EventType, ScheduledFuture> endRegTasks;
    private boolean launcheAnnounce = false;
    private int holderCounter = 0;

    private EventManager() {
        this.events = new EnumMap<>(EventType.class);
        this.endRegTasks = new EnumMap<>(EventType.class);
        this.playedPlayers = new EnumMap<>(EventType.class);
        this.holders = new FastList<>();
    }

    public void onEnterWorldAnnounce(final Player player) {
            ...
            public void run() {
                if (hasActiveEvents()) {
                    ...
                    for (Entry<EventType, Boolean> entry : events.entrySet()) {
                        if (entry.getValue()) {
                            sb.append("- ").append(ColorUtil.convertFromUTF8(entry.getKey().getEventTemplate().getEventName())).append("\n");
                        }
                    }
                    sb.append("Регистрация командой: .event\n").append("==============================");
    ...
    private IEventHolder createEventHolder(int index, EventType etype, EventPlayerLevel epl) {
        switch (etype) {
            case E_1x1:
                return new _1x1SamePlayerClassEventHolder(index, etype, epl);
            case E_LHE:
    ...
    private EventRegisterInfo standartCheck(EventType etype) {
        ...
        /*if (CustomConfig.NEW_PVP_MODE && etype == EventType.E_1x1) {
         return null;
         }*/
        ...
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (hasActiveEvents()) {
                    ...
                    for (Entry<EventType, Boolean> entry : events.entrySet()) {
                        if (entry.getValue()) {
                            if (entry.getKey() == EventType.E_LHE) {
                                LHE = true;
                            }
                            sb.append("- ").append(ColorUtil.convertFromUTF8(entry.getKey().getEventTemplate().getEventName())).append("\n");
                        }
                    ...
                    sb.append("Регистрация командой: .event\n").append("==============================");
                    final boolean ll = LHE;
                    World.getInstance().doOnAllPlayers(new Visitor<Player>() {
                        @Override
                        public void visit(Player object) {
                            PacketSendUtility.sendMessage(object, sb.toString());
                            if (ll) {
                                PacketSendUtility.sendBrightYellowMessageOnCenter(object, "Открыта регистрация на ивент \"Последний Герой\" регистрация командой .event reg тип_ивента");
                            }
                        }
                    });
                }
