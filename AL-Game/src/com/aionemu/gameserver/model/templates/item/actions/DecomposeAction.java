/*
 * This file is part of [JS]Emulator <www.[JS]Emulator>.
 *
...
public class DecomposeAction extends AbstractItemAction {
    ...
    private static Map<Race, int[]> chunkEarth = new EnumMap<>(Race.class);

    static {
        chunkEarth.put(Race.ASMODIANS, new int[]{
            152000051, 152000052, 152000053, 152000451, 152000453, 152000551,
        ...
        chunkEarth.put(Race.ELYOS, new int[]{
            152000001, 152000002, 152000003, 152000401, 152000403, 152000501,
    ...
    private static Map<Race, int[]> chunkSand = new EnumMap<>(Race.class);

    static {

        chunkSand.put(Race.ASMODIANS, new int[]{
            152000452, 152000454, 152000301, 152000302, 152000303, 152000456,
        ...
        chunkSand.put(Race.ELYOS, new int[]{
            152000402, 152000404, 152000301, 152000302, 152000303, 152000406,
    ...
    private static int[] chunkRock
            = {
                152000106, 152000206, 152000308, 152000310, 152000312, 152000109,
    ...
    private static int[] chunkGemstone
            = {
                152000112, 152000213, 152000116, 152000212, 152000217, 152000326,
                152000327, 152000328
            };

    private static int[] scrolls
            = {
                164002002, 164002058, 164002010, 164002056, 164002057, 164002003, 164002059,
                164002011, 164002004, 164002012, 164002012, 164000122, 164000131, 164000118
            };

    private static int[] potion
            = {
                162000045, 162000079, 162000016, 162000021, 162000015, 162000027, 162000020,
            ...
            public void run() {
                ...
                if (validAction) {
                    if (selectedCollection.getItems().size() > 0) {
                        Collection<ResultedItem> itemByType = new ArrayList<>();

                        for (ResultedItem resultItem : selectedCollection.getItems()) {
                            if (canAcquire(player, resultItem)) {
                                if (DataManager.DECOMPOSABLE_ITEMS_DATA.getItemByType(parentItem.getItemId()) == DecomposableType.JOBDROP) {
                                    itemByType.add(resultItem);
                                    continue;
                                }
                                ItemService.addItem(player, resultItem.getItemId(), resultItem.getResultCount(), AddItemType.USEITEM, "decompasible: " + parentItem.getItemId());
                            }
                        }
                        ...
                        for (RandomItem randomItem : selectedCollection.getRandomItems()) {
                            ...
                            if (randomType != null) {
                                ...
                                if (randomId != 0 && randomId != 167000524) {
                                    ItemService.addItem(player, randomId, randomItem.getResultCount(), AddItemType.USEITEM, "decompasible: " + parentItem.getItemId());
                                }
                            }
                        }
                    }
