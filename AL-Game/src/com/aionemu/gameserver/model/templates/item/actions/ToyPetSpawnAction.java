...
public class ToyPetSpawnAction extends AbstractItemAction {
    ...
    public boolean canAct(Player player, Item parentItem, Item targetItem) {
        ...
        if (player.getWorldId() == 600010000 || player.getWorldId() == 300030000) {
            PacketSendUtility.sendMessage(player, "Невозможно");
            return false;
        }
        if (player.isInsideDisablePvpZone()) {
            PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300805));
            return false;
        }
