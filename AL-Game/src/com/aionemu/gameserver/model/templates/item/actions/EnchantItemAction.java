    ...
    public void act(final Player player, final Item parentItem, final Item targetItem, final Item supplementItem,
        ...
        PacketSendUtility.broadcastPacketAndReceive(player,
                new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemTemplate()
                        .getTemplateId(), player.getMembership() >= EnchantsConfig.ENCHANT_TIME_MEMBERSHIP_LEVEL ? EnchantsConfig.ENCHANT_MEMBERSHIP_TIME : EnchantsConfig.ENCHANT_TIME, 0, 0));
        player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (CustomConfig.ENABLE_ENCHANT_ANNOUNCE) {
                    if ((itemTemplate.getCategory() == ItemCategory.ENCHANTMENT) && targetItem.getEnchantLevel() > 0 && isSuccess) {
                        ...
                        while (iter.hasNext()) {
                            ...
                            if (player2.isGM()) {
                                PacketSendUtility.sendMessage(player2, player.getName() + " enchant [item:" + targetItem.getItemId() + "] to level " + targetItem.getEnchantLevel());
                            }
                        }
                    }
                }
        ...
        }, player.getMembership() >= EnchantsConfig.ENCHANT_TIME_MEMBERSHIP_LEVEL ? EnchantsConfig.ENCHANT_MEMBERSHIP_TIME : EnchantsConfig.ENCHANT_TIME));
    }
