...
public final class PlayerSkillList implements SkillList<Player> {
    ...
    public boolean addGameMasterSkill(Player player, int skillId, int skillLevel) {
    ...
    public void addStigmaSkill(Player player, List<StigmaSkill> skills, boolean equipedByNpc) {
        for (StigmaSkill sSkill : skills) {
            ...
            if (equipedByNpc) {
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(skill, 1300401, false, false));
            }
        }
    }
    ...
    private void sendMessage(Player player, int skillId, boolean isNew) {
        switch (skillId) {
            case 30001:
            case 30002:
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330005, false, false));
                break;
            case 30003:
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330005, false, false));
                break;
            ...
            case 40009:
            case 40010:
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1330061, false, false));
                break;
            default:
                PacketSendUtility.sendPacket(player, new SM_SKILL_LIST(player.getSkillList().getSkillEntry(skillId), 1300050, isNew, false));
        }
    }

    public void removeAllGameMasterSkills(int level) {
        for (int id : AccessLevelEnum.getAlType(level).getSkills()) {
            removeSkill(id);
        }
    }
}
