...
public abstract class CreatureLifeStats<T extends Creature> {
    ...
    public int getMaxHp() {
        ...
        //WTF?
        /*if (this.getOwner() instanceof SiegeNpc) {
         maxHp /= 3;
         }*/
        if (maxHp == 0) {
            maxHp = 1;
    ...
    public final void restoreHp() {
        increaseHp(TYPE.HP, getOwner().getGameStats().getHpRegenRate().getCurrent());
    }
    ...
    public final void restoreMp() {
        increaseMp(TYPE.MP, getOwner().getGameStats().getMpRegenRate().getCurrent());
    }
