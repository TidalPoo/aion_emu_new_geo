...
public class House extends VisibleObject {
    ...
    public synchronized void spawn(int instanceId) {
        ...
        if (getPosition() == null || !getPosition().isSpawned()) {
            WorldPosition position2 = World.getInstance().createPosition(address.getMapId(), address.getX(), address.getY(), address.getZ(),
                    (byte) 0, instanceId);
    ...
    public float getVisibilityDistance() {
        return (HousingConfig.VISIBILITY_DISTANCE * 1f);
    }
