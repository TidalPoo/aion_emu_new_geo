...
public class Equipment {
    ...
    public void expireUnEquipItem(int itemUniqueId, long slot) {
        synchronized (equipment) {
            Item itemToUnequip = null;
            ...
            if (itemToUnequip.getEquipmentSlot() == ItemSlot.MAIN_HAND.getSlotIdMask()) {
                Item ohWeapon = equipment.get(ItemSlot.SUB_HAND.getSlotIdMask());
                if (ohWeapon != null && ohWeapon.getItemTemplate().isWeapon()) {
                    subHandSlot = ItemSlot.SUB_HAND.getSlotIdMask();
                    unqId = ohWeapon.getObjectId();
                    unEquip(subHandSlot);
                }
            }
            ...
            unEquip(itemToUnequip.getEquipmentSlot());
            if (subHandSlot != 0) {
                equipItem(unqId, subHandSlot);
                PacketSendUtility.broadcastPacket(owner, new SM_UPDATE_PLAYER_APPEARANCE(owner.getObjectId(),
                        owner.getEquipment().getEquippedForApparence()), true);
            }
        }
    }
    ...
    public boolean isWeaponEquipped(WeaponType weaponType) {
        ...
        return equipment.get(ItemSlot.SUB_HAND.getSlotIdMask()) != null
    ...
    public boolean isUsedTwoHandWeapon() {
        Item main = getMainHandWeapon();
        if (main != null) {
            return main.getItemTemplate().isTwoHandWeapon();
        }
        return false;
    }
