...
public class QuestEngine implements GameEngine {
    ...
    public boolean onAtDistance(QuestEnv env) {
        ...
        if (getQuestNpc(npc.getNpcId()).getOnDistanceEvent().isEmpty()) {
            return false;
        }
        Player player = env.getPlayer();
    ...
    public void registerOnKillRanked2(AbyssRankEnum playerRank, int questId) {
        TIntArrayList onKillRankedQuests = new TIntArrayList();
        onKillRankedQuests.add(questId);
        questOnKillRanked.put(AbyssRankEnum.getRankById(playerRank.getId()), onKillRankedQuests);
    }
    ...
    public void printMissingSpawns() {
        ...
        for (QuestHandler handler : questHandlers.values()) {
            ...
            HashSet<Integer> allNpcs = handler.getNpcIds();
