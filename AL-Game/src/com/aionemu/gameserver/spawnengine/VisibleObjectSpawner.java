...
public class VisibleObjectSpawner {
    ...
    protected static VisibleObject spawnNpc(SpawnTemplate spawn, int instanceIndex) {
        ...
        //TODO
        boolean testSpawner = false;
        if (testSpawner) {
            for (Npc n : World.getInstance().getNpcs()) {
                double i = MathUtil.getDistance(n, npc);
                if (n.getNpcId() == npc.getNpcId() && n.getWorldId() == npc.getWorldId() && n.getX() == npc.getX() && n.getY() == npc.getY() && n.getZ() == npc.getZ()
                        || n.getNpcId() == npc.getNpcId() && n.getWorldId() == npc.getWorldId() && i >= 0 && i < 3) {
                    n.getController().onDelete();
                    //World.getInstance().despawn(npc);
                    log.info("1: " + n.getNpcId());
                }
            }
        }
