...
public abstract class CreatureController<T extends Creature> extends VisibleObjectController<Creature> {
    ...
    /**
     * Обновить полностью зону, независимо от текущей зоны
     */
    ...
    public void onDie(Creature lastAttacker) {
        ...
        if (getOwner() instanceof Player && (NewPlayerZone.isInZone((Player) getOwner()) || FfaPlayers.isInFFA((Player) getOwner()) || FfaGroupService.isInFFA((Player) getOwner()) || FfaLegionService.isInFFA((Player) getOwner())
                || getOwner().getPosition().getWorldMapInstance().getInstanceHandler() instanceof GeneralEventHandler || MixFight.isInFFA((Player) getOwner()))) {
            return;
        }
    ...
    public void onAttack(final Creature attacker, int skillId, TYPE type, int damage, boolean notifyAttack, LOG log) {
        ...
        //Если у атакуемого игрока нет таргета атакующего, добавить его атакуемому.
