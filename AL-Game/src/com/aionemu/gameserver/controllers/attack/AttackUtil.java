...
public class AttackUtil {
    ...
    public static List<AttackResult> calculateMagicalAttackResult(Creature attacker, Creature attacked, SkillElement elem) {
        ...
        int damage = StatFunctions.calculateAttackDamage(attacker, attacked, true, elem);
        List<AttackResult> attackList = new ArrayList<>();
        AttackStatus mainHandStatus = calculateMagicalMainHandResult(attacker, attacked, attackerStatus, damage, attackList);

        if (attacker instanceof Player && ((Player) attacker).getEquipment().getOffHandWeaponType() != null) {
            calculateMagicalOffHandResult(attacker, attacked, mainHandStatus, attackList, elem);
        }
        attacked.getObserveController().checkShieldStatus(attackList, null, attacker);
        return attackList;
    ...
    /**
     * Calculate magical attack status and damage of the MAIN hand
     */
    private static AttackStatus calculateMagicalMainHandResult(Creature attacker, Creature attacked,
            AttackStatus attackerStatus, int damage, List<AttackResult> attackList) {
        AttackStatus mainHandStatus = attackerStatus;
        if (mainHandStatus == null) {
     ...
     * Calculate physical attack status and damage of the OFF hand
     */
    private static void calculateMagicalOffHandResult(Creature attacker, Creature attacked, AttackStatus mainHandStatus,
     ...
     * Generate attack results based on weapon hit count
     */
    private static List<AttackResult> splitMagicalDamage(final Creature attacker, final Creature attacked,
            int damage, AttackStatus status, List<AttackResult> attackList) {
