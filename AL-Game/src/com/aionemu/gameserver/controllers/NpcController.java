...
public class NpcController extends CreatureController<Npc> {
    ...
    public void onDespawn() {
        ...
        /*if (CustomConfig.NEW_PVP_MODE) {
         int i = owner.getInstanceId();
         boolean ffa = i == FfaGroupService.getFFAInstanceId() || i == FfaLegionService.getFFAInstanceId() || i == NewPlayerZone.getFFAInstanceId();
         if (getOwner().getNpcObjectType() != NpcObjectType.SUMMON && ffa) {
         if (!owner.isDeleteDelayed()
        ...
        if (owner.isNoRespawn() && getOwner().getNpcObjectType() != NpcObjectType.SUMMON && !owner.isInInstance()) {
    ...
    public void onDie(Creature lastAttacker) {
        ...
        owner.setNpcType(CreatureType.AGGRESSIVE.getId());
        ...
        for (AggroInfo info : finalList) {
            ...
            if (attacker instanceof TemporaryPlayerTeam<?>) {
                ...
                if (!player.getLifeStats().isAlreadyDead()) {
                    player.increaseNpcKills();
                    if (player.getLevel() == getOwner().getLevel() || getOwner().getLevel() == 65) {
                        player.increaseSpecialKillsMob();
                    }
                    // Reward init
                    ...
                    if (getOwner().isRewardAP()) {
                        ...
                        if (rewardAp > 0) {
                            // Костыль: если рейтинг нпс босс и хп меньше 150к устанавливать выдачу ап 60
                            if (rewardAp > 100 && getOwner() instanceof SiegeNpc && getOwner().isBoss()) {
                                int maxHp = CustomConfig.MAX_HP_NPC;//350000
                                if (getOwner().getLifeStats().getMaxHp() <= maxHp) {
                                    rewardAp = 60;
                                }
                            }
                    ...
                    int toll = getOwner().getObjectTemplate().getToll();
                    int kinah = getOwner().getObjectTemplate().getKinah();
                    int locId = getOwner().getObjectTemplate().getLocId();
                    if (toll > 0) {
                        if (locId == 0 || locId > 0 && getOwner().getWorldId() == locId) {
    ...
    @Override
    public void onDialogSelect(int dialogId, final Player player,
            int questId, int extendedRewardIndex
    ) {
