...
public class PlayerController extends CreatureController<Player> {
    ...
    public void updateNearbyQuests() {
        ...
        if (getOwner().getPosition() != null && getOwner().getPosition().getMapRegion() != null) {
    ...
    public void onEnterZone(ZoneInstance zone) {
        ...
        if (!zone.canRide() && player.isInPlayerMode(PlayerMode.RIDE) && !player.isGM()) {
            player.unsetPlayerMode(PlayerMode.RIDE);
        }
        InstanceService.onEnterZone(player, zone);
        //player.setSaveZonePosition(player.getWorldId(), player.getInstanceId(), player.getX(), player.getY(), player.getZ());
    ...
    public void onEnterWorld() {
        Player player = getOwner();
        //ffa
        FfaPlayers.onEnterFFA(getOwner());
        FfaGroupService.onEnterFfa(player);
        FfaLegionService.onEnterFfa(player);
        InstanceService.onEnterInstance(getOwner());

        //для телепорта на вход в локацию если упал в текстуры :D
        ...
        for (Effect ef : getOwner().getEffectController().getAbnormalEffects()) {
            if (ef.isDeityAvatar()) {
                ...
                if (getOwner().getWorldType() != WorldType.ABYSS && getOwner().getWorldType() != WorldType.BALAUREA
                        || getOwner().isInInstance() || FfaGroupService.isInFFA(getOwner()) || FfaPlayers.isInFFA(getOwner()) || MixFight.isInFFA(getOwner())) {
                    ef.endEffect();
        ...
        getOwner().setUnderNoFPConsum(getOwner().isInWorldNoFPReduce());
    }
    ...
    public void onLeaveWorld() {
        ...
        FfaPlayers.onLeaveFFA(getOwner());
        FfaGroupService.onLeaveFfa(getOwner());
        FfaLegionService.onLeaveFfa(getOwner());
        PvPLocationService.onLeavePvPLocation(getOwner(), false);
    }
    ...
    public void onDie(@Nonnull Creature lastAttacker, boolean showPacket) {
        ...
        player.getCommonData().increaseDie();
        ...
        if (!FfaPlayers.isInFFA(getOwner()) && !FfaGroupService.isInFFA(player) && !FfaLegionService.isInFFA(player) && !MixFight.isInFFA(getOwner()) && !NewPlayerZone.isInZone(getOwner())) {
        ...
        if (CustomConfig.ENABLE_FULL_DROP_PLAYER && !player.isGM()) {
            PlayerDropService.getInstance().onDie(player);
        }
    }
    ...
    public void attackTarget(Creature target, int time) {
        ...
        if (!MathUtil.isInAttackRange(getOwner(), target,
                getOwner().getGameStats().getAttackRange().getCurrent() / 1000f + 1)) {
            return;
        }
    ...
    public void onAttack(Creature creature, int skillId, TYPE type, int damage, boolean notifyAttack, LOG log) {
        ...
        /*if (creature instanceof Kisk) {
         if (!creature.getMaster().isEnemyFrom(getOwner())) {
         return;
         }
         }*/
        cancelUseItem();
        ...
        //PacketSendUtility.broadcastPacket(getOwner(), new SM_ATTACK_STATUS(getOwner(), TYPE.MP, skillId, -damage, LOG.SPELLATK), true);
        ...
        if (getOwner().isInvul() && creature instanceof Player && getOwner() != creature) {
            PacketSendUtility.sendMessage((Player) creature, "Immortal Object");
        }
    }
    ...
    public void useSkill(SkillTemplate template, int targetType, float x, float y, float z, int clientHitTime,
        ...
        if (skill == null && player.isTransformed()) {
            ...
            if (panel != null && panel.canUseSkill(template.getSkillId(), skillLevel)) {
                ...
                //PacketSendUtility.sendMessage(player, "panel_skills = \"" + "(" + template.getSkillId() + ")" + (template.getSkillId() << 8) + "\"");
            }
        }
    ...
    public void upgradePlayer() {
        ...
        if (!this.getOwner().getCommonData().isChangeClass()) {
            // add new skills
        ...
        /*if (player.getLevel() == 60 && KarunItemToLvl.addItem(player)) {
         KarunItemToLvl.addItems(player);
         }*/
    ...
    public boolean addItems(int itemId, int count, int questId) {
        return ItemService.addQuestItems(getOwner(), Collections.singletonList(new QuestItems(itemId, count, questId)));
    }
    ...
    public static interface ItemListener {
        ...
        void onPlayerUsedItem(int itemid, Player player);
    }
