    ...
    protected void writeImpl(AionConnection con) {
        ...
        switch (type) {
            ...
            case 1:
                break;
            case 2:
                int count = SecurityConfig.PASSKEY_WRONG_MAXCOUNT;
                if (wrongCount > 0) {
                    count = SecurityConfig.PASSKEY_WRONG_MAXCOUNT - wrongCount;
                }
                writeH(messageType); // 0: newpasskey complete, 2: passkey edit complete, 3: passkey input
                writeC(wrongCount > 0 ? 1 : 0); // 0: right passkey, 1: wrong passkey показать окно с попытками
                writeD(count); // сколько осталось попыток
                writeD(SecurityConfig.PASSKEY_WRONG_MAXCOUNT); // Enter the number of possible wrong numbers (retail
                // server default value: 5) макс кол-во попыток...
                break;
        }
    }
}
