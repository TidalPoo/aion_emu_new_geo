...
public class SM_NPC_INFO extends AionServerPacket {
    ...
    public SM_NPC_INFO(Npc npc, Player player) {
        ...
        npcTypeId = npc.getLifeStats().isAlreadyDead() ? CreatureType.AGGRESSIVE.getId() : npc.getType(player);
        npcId = npc.getNpcId();
    ...
    protected void writeImpl(AionConnection con) {
        ...
        if (gear == null) {
            ...
            // actually use items, we'll make Item from it )
            {
                ...
                writeD(item.getValue().getItemSkinId());
                writeD(0x01);
                writeD(0x00);
                writeH(0x00);
            }
        ...
        //_npc.isFlag() ? 0x13 : /*_npc.isNewSpawn() ? 0x01 : */ 0x00
        writeC(_npc.isFlag() ? 0x13 : _npc.isNewSpawn() ? 0x01 : 0x00); // animation spawn if isFlag npc
