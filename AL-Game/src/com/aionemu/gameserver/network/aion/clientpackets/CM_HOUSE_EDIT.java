...
public class CM_HOUSE_EDIT extends AionClientPacket {
    ...
    protected void runImpl() {
        ...
        PacketSendUtility.sendMessage(player, "CM_H_EDIT [item:" + itemObjectId + "]\nBuildId: " + buildingId + " action: " + action);
    }

    private boolean removeRenovationCoupon(Player player, House house) {
        int typeId = house.getHouseType().getId();
        if (typeId == 0) {
            return false; // studio, wtf ???
        }
        if (player.isDeveloper() || player.isGM()) {
            return true;
        }
        /* if (HousingConfig.ENABLE_FREE_RENOVATION_TO_MEMBERSHIP){
