...
public class CM_CHARACTER_EDIT extends AionClientPacket {
    ...
    private boolean have_edit = true;
    private boolean have_character = true;
    ...
    protected void readImpl() {
        ...
        have_edit = !player.havePermission((byte) MembershipConfig.EDIT_GENDER);
        have_character = !player.havePermission((byte) MembershipConfig.EDIT_CHARACTER);
        if (player.getAccessLevel() == 0 && !player.isDeveloper()) {
            if (have_character && !gender_change || have_edit && gender_change) {
                if (!gender_change) {
                    //внешности
    ...
    protected void runImpl() {
        ...
        if (!check_ticket) {
            ...
            if (!gender_change) {
                if (have_character && player.getAccessLevel() == 0) {
                ...
                if (have_edit && player.getAccessLevel() == 0 && !player.isDeveloper()) {
