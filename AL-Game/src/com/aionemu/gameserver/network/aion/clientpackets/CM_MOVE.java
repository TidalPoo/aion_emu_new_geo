...
public class CM_MOVE extends AionClientPacket {
    ...
    protected void readImpl() {
        ...
        if (player == null || !player.isSpawned() && !player.isFlying()) {
            return;
        }
        ...
        heading = (byte) readC();
        type = (byte) readC();
        //PacketSendUtility.sendMessage(player, "type " + type + " h " + heading + " x " + x + " y " + y + " z " + z);
    ...
    protected void runImpl() {
        ...
        if (player.getEffectController().isAbnormalState(AbnormalState.CANT_MOVE_STATE) || player.getEffectController().isUnderFear()) {
            PacketSendUtility.broadcastPacketAndReceive(player,
                    new SM_FORCED_MOVE(player, player.getObjectId(), player.getX(), player.getY(), player.getZ()));
            return;
        }
        ...
        //todo
        /*if (player.isFlying() && !player.getFlyController().isChecker()) {
