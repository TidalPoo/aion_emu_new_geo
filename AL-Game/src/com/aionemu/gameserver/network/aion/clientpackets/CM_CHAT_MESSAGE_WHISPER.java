...
public class CM_CHAT_MESSAGE_WHISPER extends AionClientPacket {
    ...
    protected void runImpl() {
        ...
        Player receiver = World.getInstance().findPlayer(name);

        if (LoggingConfig.LOG_CHAT) {
            log.info(String.format("[MESSAGE] [%s] Whisper To: %s, Message: %s", sender.getName(), name, message));
        }

        int level = 10;
        if (receiver == null) {
            sendPacket(SM_SYSTEM_MESSAGE.STR_NO_SUCH_USER(name));
        } else if (!receiver.isWispable()) {
            //STR_WHISPER_REFUSE
            //Персонаж %0 отключил прием приватных сообщений.
            //1300629
            PacketSendUtility.sendPacket(sender, new SM_SYSTEM_MESSAGE(1300629, name));
            ...
            sendPacket(SM_SYSTEM_MESSAGE.STR_NO_SUCH_USER(name));
        } else if (sender.isInFfa() && receiver.isInFfa() && !sender.isGM() && !sender.isDeveloper()) {
            PacketSendUtility.sendMessage(sender, "Нельзя пользоватся чатом в FFA зоне");
        } else if (sender.getLevel() < level) {
            PacketSendUtility.sendPacket(sender, new SM_SYSTEM_MESSAGE(1310004, level));
        } else {
            //if (RestrictionsManager.canChat(sender))
            PacketSendUtility.sendPacket(receiver, new SM_MESSAGE(sender, NameRestrictionService.filterMessage(message, sender), ChatType.WHISPER));
            AdminSniffChatMessage.getInstance().sendMessage(sender, ChatType.WHISPER, message);
        }
    }
