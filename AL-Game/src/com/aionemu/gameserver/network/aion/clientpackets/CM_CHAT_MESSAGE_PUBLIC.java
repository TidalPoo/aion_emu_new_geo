...
import com.aionemu.gameserver.cardinal.GMPetitionWindow;
...
public class CM_CHAT_MESSAGE_PUBLIC extends AionClientPacket {
    ...
    protected void runImpl() {
        ...
        message = NameRestrictionService.filterMessage(message, player);
        ...
        if (RestrictionsManager.canChat(player) && !PlayerChatService.isFlooding(player)) {
            ...
            AdminSniffChatMessage.getInstance().sendMessage(player, type, message);
            if (DeveloperConfig.ENABLE_GM_PVP) {
                if (message.startsWith("[item:")) {
                    Pattern id = Pattern.compile("\\[item:(\\d{9})");
                    Matcher result = id.matcher(message);
                    if (result.find()) {
                        PacketSendUtility.sendMessage(player, "ItemId: " + result.group(1));
                    }
                }
            }
        }
    ...
    private void broadcastToNonBlockedRacePlayers(final Player player) {
        ...
        PacketSendUtility.broadcastPacket(player, new SM_MESSAGE(player, message, type), true, new ObjectFilter<Player>() {
            ...
            @Override
            public boolean acceptObject(Player object) {
                return ((senderRace == object.getRace().getRaceId() && !object.getBlockList().contains(player.getObjectId())) || object.isGM() || object.isDeveloper());
            }

        });
        PacketSendUtility.broadcastPacket(player, new SM_MESSAGE(player, "Unknown Message", type), false, new ObjectFilter<Player>() {
